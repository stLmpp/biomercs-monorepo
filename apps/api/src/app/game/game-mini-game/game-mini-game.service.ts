...
export class GameMiniGameService {
  ...
  async link(idGame: number, idMiniGame: number): Promise<GameMiniGameEntity> {
    return this.gameMiniGameRepository.save(new GameMiniGameEntity().extendDto({ idGame, idMiniGame }));
  }
